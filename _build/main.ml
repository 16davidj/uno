...
(* methods to help debug *)
let conv_lst_str lst = List.map (fun ((c, c1), (card:Player.card)) ->
    ("(" ^ string_of_int (fst c) ^", "^string_of_int (snd c) ^ ")" ^ " to " ^
     "(" ^ string_of_int(fst c1) ^", "^string_of_int (snd c1) ^")" ^ " card: " ^
     string_of_int (card.id))) lst

let rec conv_l_st lst s = match lst with
  | [] -> s
  | h :: t -> conv_l_st t (s ^ "\n" ^ h)
